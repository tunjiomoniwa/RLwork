...
\emph{\textbf{Goal}}: The goal is for the agent to learn to minimize the overall cost~$\mathcal{C}$ in the tuple $\langle$Outage communication cost ($\mathcal{P}_{out}$) /Energy of the fog relay (J) /Energy of the IoT sensor (J)$\rangle$, by keeping all nodes within the link alive while ensuring that the communication outage does not fall below the pre-defined threshold, which was set at 5\%.
