...
\emph{\textbf{States}}: The states are defined as a tuple, $\langle$Outage communication cost ($\mathcal{P}_{out}$) /Energy of the fog relay (J) /Energy of the IoT sensor (J)$\rangle$.

\begin{itemize}
  \item Outage communication cost: This observation will be evaluated based on the cost function in (\ref{eqn1a}), which gives an estimate of the communication outage when there is an action, such as, change in the power level or location, or both.
  \item Energy of the fog relay: This observation gives the agent insight on how much energy has been expended on mobility.
  \item Energy of the IoT sensor: This observation gives the agent insight on how much energy the IoT sensor has used up due to high-power transmission.
\end{itemize}

\begin{equation}\label{eqn1a}
  \mathcal{P}_{out} = 1 - (1 + 2\Psi^2 \ln \Psi) \exp\Big( -\frac{N_0 \tilde{\gamma}}{P_{I} (D_{I} + \delta)^{-\alpha}}\Big),
\end{equation}

where~$\Psi = \sqrt{(N_0 \tilde{\gamma})/(P_{R}(D_{S} + \delta)^{-\alpha})}$, and~$\mathcal{P}_{out}$ is an expression for the outage probability with values between 0 and 1. We assume a predefined threshold~$\tilde{\gamma}$ which determines the outage in communication,~$P_{I}$ is transmit power of the IoT sensor,~$P_{R}$ is transmit power of the fog relay agent,~$D_{I}$ is the distance between IoT sensor and fog relay agent, and~$D_{S}$ is the distance between fog relay agent and destination node. We assume a small change in the position of the fog relay agent,~$\delta = 0.1m$,~$N_0$ to be the channel noise, and~$\alpha$ to be the path-loss exponent.

\emph{\textbf{Actions}}: The actions are a tuple, $\langle$Move and transmit/power-level of IoT end-device$\rangle$.

 \begin{itemize}
  \item Mobility: Move by $\pm \delta$ and transmit, where~$\delta = 0.1m$ and mobility range = [-20m, 20m]
  \item Power-level: Choose power-level and transmit, $\rho$, where~transmit power ranges = [0.1W, 0.2W, 0.3W, 0.4W, 0.5W, 0.6W, 0.7W, 0.8W, 0.9W, 1W]
\end{itemize}

\emph{\textbf{Goal}}: The goal is for the agent to learn to minimize the overall cost~$\mathcal{C}$ in the tuple $\langle$Outage communication cost ($\mathcal{P}_{out}$) /Energy of the fog relay (J) /Energy of the IoT sensor (J)$\rangle$.

\emph{\textbf{Rewards}}: We may consider the reward function in (\ref{eqn1b})

\begin{equation}\label{eqn1b}
  R  = 100\Big[(1 - \mathcal{P}_{out}) + (1 - \frac{EU_{fog}}{C_{fog}}) + (1 - \frac{EU_{IoT}}{C_{IoT}})\Big],
\end{equation}

where~$EU_{fog}$ and~$EU_{IoT}$ are the energy used up by the fog agent and the IoT sensor,~$C_{fog}$ and~$C_{IoT}$ are the initial capacity of the fog agent and the IoT sensor, respectively.

Metrics: Outage probability, i.e. the ratio of the number of packet lost to those transmitted, and Energy in Joules.
...
%\begin{figure}[!t]
%\centering
%\includegraphics[width=3.1in]{energy_saved.eps}
% %where an .eps filename suffix will be assumed under latex,
%% and a .pdf suffix will be assumed for pdflatex; or what has been declared
%% via \DeclareGraphicsExtensions.
%\caption{energy saved over episodes.}
%\label{energy_saved}
%\end{figure}
...
%\begin{figure}[!t]
%\centering
%\includegraphics[width=3.1in]{mdp.eps}
% %where an .eps filename suffix will be assumed under latex,
%% and a .pdf suffix will be assumed for pdflatex; or what has been declared
%% via \DeclareGraphicsExtensions.
%\caption{MDP of proposed model showing possible action range.}
%\label{mdp}
%\end{figure}
%
%\begin{figure}[!t]
