...
class FogIoT:
    def __init__(self, deltaval, pw1, pw2, pw3, pw4, pw5, pw6):
        ...
        #new attributes
        self.obs = np.array([np.random.randint(0, 80), np.random.randint(95, 100), np.random.randint(95, 100)])
        self.ECI = 0
        self.ECF = 0
        self.sum_pack = 0
        self.final_pack = 0
        ...
        self.outage, self.ef, self.ei = self.obs
        self.done = bool(self.outage< self.goal_outage and self.ef>self.goal_ef and self.ei> self.goal_ei)
        self.dead = bool(self.ef == 0 or self.ei == 0)
    ...
    def step(self, action):
        ...
        if action==0:
            ...
            self.ECI= self.ed8
        else: #not moving or transmiting
            self.ECF= self.ed9
            self.ef -= self.ECF
...
##        self.ei = np.clip(self.ei, self.min_energy_IoT, self.max_energy_IoT)
        ...
        self.obs = (self.outage, self.ef, self.ei)
...
for epi in range(episodes):
    ...
    for agent in listagent:
        ...
        agent.obs = np.array([np.random.randint(0, 80), np.random.randint(95, agent.max_energy_fog), np.random.randint(95, agent.max_energy_IoT)])
        

        agent.cur_action = agent.action_space.sample()
        agent.obs, agent.reward, agent.done, agent.dead, _ = agent.step(agent.cur_action)
        ###      
    agent.sum_pack = 0
    ...
    while ((iter < iteration_steps) and  not agent.done):#(current_state[0]>=8 and current_state[1]>0 and current_state[2]> 0)): #current_state[0]!= 0):
        listagent = ['a','b']
        iter+=1
        ...
        for agent in listagent:
            ...
            agent.obs, agent.reward, agent.done, agent.dead, _ = agent.step(agent.actionx)
            print(agent.obs)
        ...
        #save current state
            ...
            if agent.dead:
                ...
                break
        agent.ave_pack = agent.sum_pack/iter
        agent.final_pack = (100 - agent.obs[0])
        #print("End of episode #",epi, "  in ", iter , "iterations")
        agent.packets_holder.append(agent.ave_pack)
        agent.final_packets_holder.append(agent.final_pack)
