...
class FogIoT:
    def __init__(self, deltaval, pw1, pw2, pw3, pw4, pw5, pw6):
        ...
        #new attributes
        self.obs = np.array([np.random.randint(0, 80), np.random.randint(95, 100)])
        self.ECI = 0
        self.ECF = 0
        self.sum_pack = 0
        self.final_pack = 0
        ...
        self.outage, self.ef= self.obs
        self.done = bool(self.outage< self.goal_outage and self.ef>self.goal_ef)
        self.dead = bool(self.ef == 0)


        self.low = np.array([self.min_outage, self.min_energy_fog])
        self.high = np.array([self.max_outage, self.max_energy_fog])
    ...
    def step(self, action, IoTTrigger):
        ...
        self.obs = (self.outage, self.ef)
        return np.array(self.obs), self.reward, self.done, self.dead, {}
        

        
    def grouping(self, obs):
        upper_bounds = [self.observation_space.high[0], self.observation_space.high[1]]
        lower_bounds = [self.observation_space.low[0], self.observation_space.low[1]]
...
for epi in range(episodes):
    ...
    for agent in listagent:
        ...
        agent.obs = np.array([np.random.randint(0, 80), np.random.randint(95, agent.max_energy_fog)])
